...
import { type BreadcrumbItem, type BusinessOption, type Website } from '@/types';
import { Head, Link, useForm } from '@inertiajs/vue3';
import { computed, onBeforeUnmount, ref, watch } from 'vue';

const props = defineProps<{
    website: Pick<Website, 'id' | 'name' | 'slug' | 'primary_domain' | 'logo' | 'status' | 'business_id'>;
...
const existingLogoUrl = computed(() => logoUrl(props.website.logo));

const form = useForm({
    name: props.website.name,
    slug: props.website.slug,
    primary_domain: props.website.primary_domain ?? '',
    business_id: (props.website.business_id ?? '') as string | number,
    status: props.website.status ?? 'active',
...
const submit = () => {
    form.transform((data) => ({
        ...data,
        _method: 'put',
    })).post(route('websites.update', props.website.id), {
        forceFormData: true,
    });
};
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <div class="flex flex-1 flex-col gap-6 p-4 md:max-w-2xl">
            <HeadingSmall title="Edit website" description="Update website details" />
            ...
            <form @submit.prevent="submit" class="space-y-6">
                ...
                <div class="grid gap-2">
                    ...
                    <Input
                        ...
                        @change="onLogoChange"
                    />
                    <p class="text-sm text-muted-foreground">PNG, JPG, GIF, or WebP. Max 2MB. Leave empty to keep the current logo.</p>
                    <InputError :message="form.errors.logo" />

                    <div v-if="logoPreview || existingLogoUrl" class="w-fit">
                        <img
                ...
                <div class="flex items-center gap-4">
                    <Button type="submit" :disabled="form.processing">Update Website</Button>
